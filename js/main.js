...
class TicTacToe {
    ...
    checkForWinner() {
        for (let condition of this.winStates) {
            ...
            for (let position of condition) {
                if (this.gameState[position[0]][position[1]] != this.currentPlayer.token) { // a lot of for loops in fors
            ...
            if (winningCondition) {
                ...
                //creating win event meant create a string event.....thought it meant something else...so just do via new Event();
    ...
    recordMove(event) {
        ...
        let tile_x = event.target.dataset.x; // not sure why its just called x, i thought I would be using event.target.dataset.data-x
        let tile_y = event.target.dataset.y;


        if (!this.gameState[tile_x][tile_y]) { // time stamp 40:00 and 1:14:00
            this.gameState[tile_x] [tile_y] = this.currentPlayer.token;
            event.target.setAttribute('class', `tile played fas fa-${this.currentPlayer.token}`) // tile played fas-fa why????
            // should be: `tile played fas fa-${this.currentPlayer.token}`.
        } else {
            return false; // retunring false means to stop event,
        }
    ...
    switchPlayer() {
        ...
        this.currentPlayerToken.setAttribute('class', `fas fa-${this.currentPlayer.token}`) // fas fa- is icon singifiyer. 
    ...
    setUpTileListeners() {
        ...
        for (const tile of tileElements) { //i had trouble here.
            tile.addEventListener('click', handleMove);
        }
    }
    showWinScreen() {
        // This method displays the end game screen for a Win.
        this.winScreen.setAttribute('class', 'show'); // the class .show is on main.css for refrence on its description
    ...
    setUpBoard() {
        ...
        this.gameboard.innerHTML = ""; // set of blank slate first....
        ...
        for (let i = 0; i < 3; i++) { // setting the for loop for row x3
            ...
            for (let j = 0; j < 3; j++) { // setting for loop for column x3
                ...
                // so this was a little new in learnings. refrencing the link above for data-attributes
        ...
        this.setUpTileListeners();//31:50 time stamp
    ...
    initializeMovePrompt() {
        ...
        this.movePrompt.setAttribute('click', '');
    ...
    start() {
        ...
        console.log('initalize more prompt');
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    startButton.addEventListener('click', (event) => {
        game = new TicTacToe(); // new function slack channel does go indepth a little more
        game.start();
    });
...
document.addEventListener('win', (ecent)=>{
